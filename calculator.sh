...
DB1=$1
DB2=$2
color "33" "Executing the apphashd binary to get differing hashes"
sleep 3
apphashd $1 $2
...
color "33" "Cloning iavl repo and building iaviewer"
sleep 3
rm -rf iavl
...
## Printing the iavl trees of the affected modules using iaviewer
for module in $DIFFERING_MODULES; do
color "33" "Printing the iavl tree of $module of db1 to file"
sleep 3
touch hashes/node-1-$module-shape
iaviewer shape $DB1 s/k:$module/ > hashes/node-1-$module-shape
color "33" "Printing the iavl tree of $module of db2 to file"
sleep 3
...
while IFS= read -r line; do
  while [[ "$line" =~ ([*-][0-9]+\ )([0-9A-F]+) ]]; do
    ...
    echo "$ascii" >> hashes/decoded-$module-diff
...
color "32" "All the hashes are printed in their respective files in the hashes directory. Please check the decoded-* files to see the root cause of the apphash"
